...
bool OwlModemSSLBG96::initContext() {
  if (atModem_->doCommandBlocking("AT+QSSLCFG=\"sslversion\",0,3", 1 * 1000, nullptr) != at_result_code::OK) {
    return false;
  }

  if (atModem_->doCommandBlocking("AT+QSSLCFG=\"seclevel\",0,2", 1 * 1000, nullptr) != at_result_code::OK) {
    return false;
  }
...
bool OwlModemSSLBG96::setDeviceCert(str cert, bool force) {
  ...
  if (!force) {
    if (atModem_->doCommandBlocking("AT+QFOPEN=\"ssl_cert.pem\",2", 1 * 1000, &ssl_response) == at_result_code::OK) {
      ...
      if (atModem_->doCommandBlocking(1 * 1000, &ssl_response) != at_result_code::OK) {
...
bool OwlModemSSLBG96::setDevicePkey(str pkey, bool force) {
  ...
  if (!force) {
    if (atModem_->doCommandBlocking("AT+QFOPEN=\"ssl_pkey.pem\",2", 1 * 1000, &ssl_response) == at_result_code::OK) {
      // Just check if the file exists. A better solution would be to read out the file and compare, or use QFCRC, which
      // is only present in the newest firmware revisions
      OwlModemAT::filterResponse(s_qfopen, ssl_response, &ssl_response);
      long int fdesc = str_to_long_int(ssl_response, 10);
...
bool OwlModemSSLBG96::setServerCA(str ca, bool force) {
  ...
  if (!force) {
    if (atModem_->doCommandBlocking("AT+QFOPEN=\"ssl_cacert.pem\",2", 1 * 1000, &ssl_response) == at_result_code::OK) {
      // Just check if the file exists. A better solution would be to read out the file and compare, or use QFCRC, which
      // is only present in the newest firmware revisions
      OwlModemAT::filterResponse(s_qfopen, ssl_response, &ssl_response);
      long int fdesc = str_to_long_int(ssl_response, 10);
